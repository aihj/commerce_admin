...
const ConferenceAppRegisterList = () => {
  ...
  const [selectedOption, setSelectedOption] = useState('상품명');
...
const onSearch = () => {
  ...
  const updated = {
    ...
    currentPage: 0, 
    productName:
      selectedOption === '상품명' && trimmed !== '' ? trimmed : null,
    productIdx:
      selectedOption === '상품ID' ? extractProductIdx(trimmed) : null,
  };
...
const { data, isPending } = useQuery<ProductDTO[]>({
  ...
  return (
    ...
    <Box sx={{ ml: 2, display: 'flex', alignItems: 'flex-start', gap: 3 }}>
      <Box>
          ...
          {options.map((option) => (
            <Box
              ...
              sx={{
                ...
                cursor: 'pointer',
                backgroundColor:
                  selected === option ? '#e3f2fd' : 'transparent',
      ...
      <FormGroup sx={{ flex: 1 }}>
        <Box sx={{ display: 'flex', flexDirection: 'row', gap: 2 }}>
          <FormControlLabel
            control={
              <Radio
                checked={selectedOption === '상품명'}
                onChange={() => setSelectedOption('상품명')}
              />
            }
            label="상품명"
          />
          <FormControlLabel
            control={
              <Radio
                checked={selectedOption === '상품ID'}
                onChange={() => setSelectedOption('상품ID')}
              />
            }
            label="상품ID"
          />
        </Box>
        <Box sx={{ display: 'flex', width: '20vh', gap: 1, alignItems: 'center', mt: 1 }}>
          <TextField
            placeholder="검색어 입력"
            variant="outlined"
            size="small"
