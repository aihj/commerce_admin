...
const ConferenceAppRegisterList = () => {
  ...
  const [selectedOption, setSelectedOption] = useState<'주문자' | '주문번호'>(
    '주문자'
  );
  ...
  const onSearch = () => {
    ...
    const updated = {
      ...draft,
      // 텍스트 필드가 비었으면 둘 다 null
      userName: selectedOption === '주문자' && trimmed !== '' ? trimmed : null,
      orderId: selectedOption === '주문번호' && trimmed !== '' ? trimmed : null,
  ...
  const updateCarrier = async (orderIdx: number, courier: string, trackingNumber: string) => {
    try {
      ...
      alert(`배송 정보가 ${courier}-${trackingNumber}로 등록되었습니다`);
    } catch (error) {
    ...
    () => [
      ...
      columnHelper.accessor('orderStatus', {
        ...
        cell: (info) => {
          return (
            <DTCellBox>
              <span>{data?.orderStatus || info.getValue()}</span>
            </DTCellBox>
          );
        },
        size: 100,
      ...
      columnHelper.accessor('taxInvoiceStatus', {
        header: '세금 계산서 처리 상태',
        cell: (info) => {
          const status = info.row.original.taxInvoiceStatus || taxInvoiceStatus.none;
  ...
  return (
    ...
    >
      ...
      <Stack spacing={4}>
        ...
        <FormGroup sx={{ ml: 2 }}>
          <Box sx={{ display: 'flex', flexDirection: 'row', gap: 2 }}>
            <FormControlLabel
              control={
                <Radio
                  checked={selectedOption === '주문자'}
                  onChange={() => setSelectedOption('주문자')}
                />
              }
              label="주문자"
            />
            <FormControlLabel
              control={
                <Radio
                  checked={selectedOption === '주문번호'}
                  onChange={() => setSelectedOption('주문번호')}
                />
              }
              label="주문번호"
            />
        ...
        <Card sx={{ width: '100%', minWidth: '155vh' }}>
            ...
            >
              <Button
                ...
                sx={{
                  ...
                  fontWeight: 'bold',
                  '&:hover': {
                    backgroundColor: '#45a049', // 호버 시 더 어두운 초록색
                  },
                }}
                onClick={() => {
                   handleExport();
              ...
              <Button
                variant="contained"
                sx={{
                  backgroundColor: 'gray', // 초록색
                  color: 'white',
                  padding: '6px 24px',
                  fontSize: '16px',
                  fontWeight: 'bold',
                  '&:hover': {
                    backgroundColor: 'darkgray', // 호버 시 더 어두운 초록색
                  },
                }}
                onClick={() => {
                    handleMarkDelivered();
